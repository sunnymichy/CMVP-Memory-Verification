"""
run_pathB.py  --  classifier-evaluation orchestrator
====================================================
Regenerates EVERY classifier number in the paper from a dataset CSV and emits:
  - results_real.json : machine-readable results
  - results_real.tex  : LaTeX \\renewcommand macros
...
NOTES
-----
* The deployed model is selectable with --deployed (default XGBoost). The detailed
  per-class / confusion / calibration / ablation / learning-curve / PPV metrics are
  reported for that model.
* Temporal features F7/F8 are only meaningful on a corpus collected with multi-snapshot
  temporal change (data_collector/temporal_capture.py).
"""
...
def main():
    ...
    ap.add_argument("--deployed", default="XGBoost",
                    help="deployed model whose detailed metrics are reported (default XGBoost)")
    args = ap.parse_args()
    ...
    lines = [
        "%% Auto-generated by run_pathB.py from the released corpus.",
